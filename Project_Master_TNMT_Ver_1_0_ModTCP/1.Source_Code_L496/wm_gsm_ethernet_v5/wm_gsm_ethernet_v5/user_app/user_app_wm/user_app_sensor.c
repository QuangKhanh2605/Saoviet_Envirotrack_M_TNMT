...
Struct_SensorWarning        sSensorWarning[] = 
{
  ...
  {_SS_TEMP,    &s485Measure[_SS_TEMP].sUser, &s485Measure[_SS_TEMP].sConnect_u8, _SENSOR_CONNECT,       0,               _Cb_Handle_Detect_Power},
...
Struct_MeasureHanle         sMeasureHandle [] =
{
  ...
  {_SS_TEMP,    &s485Measure[_SS_TEMP],      &sDataSensorMeasure.sTemp,    0xFE,       0,      0xFE},       //Temperature
  ...
  {_SS_TSS,     &s485Measure[_SS_TSS],       &sDataSensorMeasure.sTSS,     0xFF,       0,      0xFF},
...
static uint8_t fevent_sensor_handle_status(uint8_t event)
{
    ...
    {
        if(s485Measure[i].nConnect_u8 == _SENSOR_DISCONNECT)
            sAverageMeasure[i].State = _E_BAO_LOI_THIET_BI;
        else
        {
            if(s485Measure[i].stateSensor == 0x03)
                sAverageMeasure[i].State = _E_HIEU_CHUAN;        
            else if(s485Measure[i].stateSensor == 0x02)
            {
                sValue = s485Measure[i].stateValue >> 4 & 0xFF;
        ...
        sAverageMeasure[i].Value_f = s485Measure[i].Value_f;
    }
...
void Init_UserSensor(void)
{
    ...
    if(*(__IO uint8_t*)(ADDR_USER_SENSOR) != FLASH_BYTE_EMPTY)
    {
        for(uint8_t i = 0; i < _END_SENSOR; i++)
        {
            s485Measure[i].sUser = *(__IO uint8_t*)(ADDR_USER_SENSOR+ 2 + i);
            if(s485Measure[i].sUser > 1)
