...
static uint8_t _Cb_Flash_Cmd_Failure(void)
{
    ...
    */
    ...
    } else if (Retry_u8 <= FLASH_MAX_RETRY_SKIP)
    {
        ...
        if (sS25FLvar.pMem_Wrtie_Retry != NULL)
            sS25FLvar.pMem_Wrtie_Retry();
...
{ 	
    ...
    do{
		...
		if(eFlash_S25FL_CheckWriteEnable(TIMEOUT_WR_ENABLE) == 1)
        {
            ...
            S25FL_Send_Byte(S25FL_CHIP_ERASE);
            /* Deselect the FLASH: Chip Select high */
            S25FL_ChipSelect(HIGH);	
            /* Wait the end of Flash writing */
            if (eFlash_S25FL_WaitForWriteEnd(TIMEOUT_ER_CHIP) == 1)
                return 1;
            else
                return 0;
        }
...
{
	uint8_t nPage = 0, nSingle = 0;
	uint8_t Retry = 0;
    
	nPage =  NumByteToWrite / S25FL_PAGE_SIZE;
	nSingle = NumByteToWrite % S25FL_PAGE_SIZE;
	...
	do
	{
		if (eFlash_S25FL_WaitForWriteEnd(TIMEOUT_WR_ENABLE) == 1)
		{
            while (nPage > 0)
            {
                if(eFlash_S25FL_PageWrite(pBuffer, WriteAddr, S25FL_PAGE_SIZE) != 1) { 
                    return false;
                }
            ...
            if (nSingle != 0) {
                if (eFlash_S25FL_PageWrite(pBuffer, WriteAddr, nSingle) != 1) {
                    return false;
                }
            }
...
{
    uint16_t i = 0;
    uint8_t aTEST_WRITE[256] = {0};
    uint8_t aTEST_READ[256] = {0};

    eFlash_S25FL_Erase_Sector(FLASH_S25FL_BASE);
...
uint8_t eFlash_Test (void)
{
    ...
    //xoa toan bo chip
    eFlash_S25FL_Erase_Chip();
    
	while (AddrTest < FLASH_S25FL_END)
	{
