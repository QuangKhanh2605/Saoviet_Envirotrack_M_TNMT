...
static uint8_t _CbFTP_GET_FILE1(sData *uart_string, uint16_t pos);
static uint8_t _CbFTP_GET_FILE2(sData *uart_string, uint16_t pos);
static uint8_t _CbFTP_GET_FILE3(sData *uart_string, uint16_t pos);
...
static uint8_t _CbHTTP_FIRM_LENGTH(sData *uart_string, uint16_t pos);
static uint8_t _CbHTTP_READ_1(sData *uart_string, uint16_t pos);
static uint8_t _CbHTTP_READ_2(sData *uart_string, uint16_t pos);

static uint8_t _CbHTTPS_OPEN_SES_1(sData *uart_string, uint16_t pos);
...
static uint8_t _CbAT_APN_AUTH (sData *uart_string, uint16_t pos)
{
    if ((strlen(sSimCommVar.sAPN.sUsername) == 0) 
        && (strlen(sSimCommVar.sAPN.sPassword) == 0))
    {
...
static uint8_t _CbHTTP_SETURL_1(sData *uart_string, uint16_t pos)
{   
    if (sSimFwUpdate.pFileURL != NULL)
        Sim_Common_Send_AT_Cmd( (uint8_t *) sSimFwUpdate.pFileURL, strlen(sSimFwUpdate.pFileURL) );
    
    Sim_Common_Send_AT_Cmd( (uint8_t*) ",", 1 );
    
    if (sSimFwUpdate.sServer.sPort != NULL)
        Sim_Common_Send_AT_Cmd( (uint8_t *) sSimFwUpdate.sServer.sPort, strlen(sSimFwUpdate.sServer.sPort) );
