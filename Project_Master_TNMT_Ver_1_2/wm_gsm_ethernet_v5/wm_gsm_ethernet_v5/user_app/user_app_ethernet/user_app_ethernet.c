...
static uint8_t _fSet_Put_File(void);
static uint8_t _fUp_Data_File(void);


static uint8_t _Cb_Put_OK (sData *pData);
static uint8_t _Cb_Upload_OK (sData *pData);
/*===================== Struct var ===================== */
...
uint8_t aETH_FTP_UPLOAD_CONN[5] = 
{
    _ETH_FTP_SET_USER,
    _ETH_FTP_SET_PASS,
    _ETH_FTP_SET_TYPE_BIN,
...
uint8_t aETH_FTP_UPLOAD_DATA[2] = 
{
    _ETH_FTP_SET_PUT_FILE,
    _ETH_FTP_UPLOAD_DATA,
};

uint8_t aETH_FTP_APPEND_DATA[2] = 
{
    _ETH_FTP_SET_PUT_FILE,
    _ETH_FTP_UPLOAD_DATA,
};
...
sCommandEth     sEthCmd[] = 
{
    ...
    {   _ETH_FTP_SET_PUT_FILE,      _fSet_Put_File,     _Cb_Put_OK,         "150"     },
    {   _ETH_FTP_UPLOAD_DATA,       _fUp_Data_File,     _Cb_Upload_OK,      "226"     },
};
	...
	switch (step)
	{
        ...
        case 1:   //dhcp v4
            ...
            switch (result)
            {
                ...
                default:
                    if (Check_Time_Out(sAppEthVar.LandMarkRun_u32, 10000) == true)
                    {
                        DHCPv4_stop();
                        cfail = 0;
                    }
		...
		default:    //success    
            ...
            if(UTIL_var.ModeConnNow_u8 == _CONNECT_FTP_UPLOAD ||
               UTIL_var.ModeConnNow_u8 == _CONNECT_FTP)
...
{ 
    ...
    switch (UTIL_var.ModeConnNow_u8)
    {
        ...
        case _CONNECT_DATA_BACKUP:
            ...
            if (sAppEthVar.rSocketClose_u8 == true) {
                sAppEthVar.rSocketClose_u8 = false;
                UTIL_Printf_Str ( DBLEVEL_M, "u_app_eth: socket closing\r\n" );
                close(SOCKET_DEMO);
                rReConn_u8 = true;
            }
            break;
...
//            break;
        ...
        case _CONNECT_FTP_UPLOAD:
            ...
            sEventAppEth[_EVENT_ETH_SOCK_CTRL].e_period = 100;

            SockCtrlState = AppEth_Socket_Control(CTRL_SOCK, sModemInfor.sServerTn.sServer.IPnum, 
                                                  sModemInfor.sServerTn.sServer.Port_u16, &AnyPortCtrl);
            ...
            if (SockCtrlState == SOCK_ESTABLISHED)
            {
                ...
                if (!sAppEthVar.ReConnect_u8)
                {
                    ...
                    qQueue_Clear(&qEthStep);
                    Pending_u8 = false;
                    AppEth_Push_Block_To_Queue(aETH_FTP_UPLOAD_CONN, sizeof(aETH_FTP_UPLOAD_CONN));
            ...
            {
                if (Check_Time_Out(LandMarkEstab_u32, ETH_MAX_TIME_CTRL_SOCK))
                {
                    LandMarkEstab_u32 = RtCountSystick_u32;
                    AnyPortCtrl = 50000;
                    rReConn_u8 = true;
            ...
            if(sMessage.Type_u8 == DATA_GPS)
            {
                if (sFTPvar.dsock_state == DATASOCK_READY)
                {
                    if (sFTPvar.RemotePort == 0 || sFTPvar.RemoteIP[0] == 0)
                        break;
                    
                    if (sFTPvar.DataSockConnected_u8 != true && sMessage.aMESS_PENDING[DATA_GPS] == true)
                    {
                        SockDataState = AppEth_Socket_Control(DATA_SOCK,sFTPvar.RemoteIP,
                                                              sFTPvar.RemotePort,&AnyPortData);
                        ...
                        {
                            sFTPvar.DataSockConnected_u8 = true;
                            sFTPvar.dsock_state = DATASOCK_START;
                            
                            qQueue_Clear(&qEthStep);
                            Pending_u8 = false;
                            AppEth_Push_Block_To_Queue(aETH_FTP_UPLOAD_DATA, sizeof(aETH_FTP_UPLOAD_DATA));
                ...
                else if (sFTPvar.dsock_state == DATASOCK_START)
                {
                    SockDataState = getSn_SR(CTRL_SOCK);
                ...
                {
                    UTIL_Printf_Str(DBLEVEL_M, "u_app_eth: sRecGPS.iSend_u16 == sRecGPS.iSave_u16\r\n");
                    close(DATA_SOCK);
                    close(CTRL_SOCK);
                    UTIL_var.ModeConnNow_u8 = UTIL_var.ModeConnLast_u8;
                }
            ...
            if (Check_Time_Out(sFTPvar.LandMark_u32, 30000))
            {
                UTIL_Printf_Str ( DBLEVEL_M, "u_app_eth: socket timeout closing\r\n" );
                close(DATA_SOCK);
                close(CTRL_SOCK);
                rReConn_u8 = true;
            }
...
static uint8_t _Cb_Send_Data (uint8_t event)
{
    if (sAppEthVar.Status_u8 != _ETH_MQTT_CONNECTED && sAppEthVar.Status_u8 != _ETH_FTP_CONNECTED) {
        return 1;
    }
...
static uint8_t _fSet_User (void)
{
    char dat[50]={0};
    if(UTIL_var.ModeConnNow_u8 == _CONNECT_FTP_UPLOAD)
        sprintf(dat,"USER %s\r\n", sModemInfor.sServerTn.sServer.aUSER);   // saoviet
    else
...
static uint8_t _fSet_Pass (void)
{
    char dat[50]={0};
    if(UTIL_var.ModeConnNow_u8 == _CONNECT_FTP_UPLOAD)
        sprintf(dat,"PASS %s\r\n", sModemInfor.sServerTn.sServer.aPASS);   // saoviet
    else
...
static uint8_t _fSet_CWD (void)
{
    ...
    if(UTIL_var.ModeConnNow_u8 == _CONNECT_FTP_UPLOAD)
    {
        if(strlen((char*)sModemInfor.sServerTn.aPATH) > 0)
            sprintf(dat,"CWD /%s\r\n", (char *)sModemInfor.sServerTn.aPATH);
        else
            sprintf(dat,"CWD /\r\n");  
    }
    else
...
static uint8_t _fUp_Data_File(void)
{
    if(sMessage.aMESS_PENDING[DATA_GPS] == true)
    {
        if (send(DATA_SOCK, sMessage.pRawData.Data_a8, sMessage.pRawData.Length_u16) <= 0) return 0;
    
        UTIL_Printf( DBLEVEL_M, sMessage.pRawData.Data_a8, sMessage.pRawData.Length_u16);
        
        uint32_t timeout = 0;
        while (!(getSn_IR(DATA_SOCK) & Sn_IR_SENDOK) && timeout++ < 1000000);
        setSn_IRCLR(DATA_SOCK, Sn_IR_SENDOK);
        
        if(timeout >= 1000000)
            sFTPvar.DataGetStart_u8 = true;

        timeout = 0;
        while (getSn_TX_FSR(DATA_SOCK) != getSn_TxMAX(DATA_SOCK) && timeout++ < 1000000);
...
static uint8_t _Cb_Upload_OK (sData *pData)
{
    if(sFTPvar.DataGetStart_u8 == false)
    {
        sFTPvar.DataGetStart_u8 = true;   
        Mem_Inc_Index_Send(&sRecGPS, 1);
        mReset_Raw_Data();
