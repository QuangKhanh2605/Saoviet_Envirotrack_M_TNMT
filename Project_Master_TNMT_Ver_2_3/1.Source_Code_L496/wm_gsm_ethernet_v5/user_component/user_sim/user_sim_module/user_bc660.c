...
/*===============define======================*/
static uint8_t _Cb_AT_Success(sData *uart_string, uint16_t pos);
static uint8_t _Cb_AT_CHECK_RSSI(sData *uart_string, uint16_t pos);
static uint8_t _Cb_AT_GET_CLOCK (sData *uart_string, uint16_t pos);
static uint8_t _Cb_AT_TCP_OPEN_2 (sData *uart_string, uint16_t pos);
static uint8_t _Cb_URC_CLOSED (sData *uart_string, uint16_t pos);
static uint8_t _Cb_NET_Get_Eng (sData *uart_string, uint16_t pos);

static uint8_t _Cb_AT_PSM_Enable (sData *uart_string, uint16_t pos);
static uint8_t _Cb_URC_WAKEUP(sData *str_Receive, uint16_t pos);
static uint8_t _Cb_URC_ATTACH(sData *str_Receive, uint16_t pos);
...
uint8_t aBC660_BLOCK_INIT[13] =
{
    ...
    _BC660_NET_SET_BAND_3,      //6
    _BC660_AT_GET_ID,			//7
    _BC660_CONF_EVENT_SLEEP,    //8
    _BC660_CONF_WAKE_PIN,       //9
	_BC660_AT_CHECK_SIM,		//10
	_BC660_AT_CHECK_RSSI,		//11
    
	_BC660_AT_SET_APN,			//12
};
...
const sCommand_Sim_Struct aBC660Urc[] =
{
    ...
    {   _BC660_URC_ATTACH, 		        _Cb_URC_ATTACH,	            "+IP: ",	  	            NULL	},
...
static uint8_t _Cb_NET_Get_Eng (sData *uart_string, uint16_t pos)
{
    char *sNext = (char *) uart_string->Data_a8;
    char aTEMP[20] = {0};
    uint8_t count = 0;
    ...
    while (count < 9)
    {
        ...
        if (sNext == NULL) {
            UTIL_Printf_Str(DBLEVEL_M, "u_bc660: qeng fail format!");
            return true;
        }
...
static uint8_t _Cb_URC_ATTACH(sData *str_Receive, uint16_t pos)
{
    sCbSimComm.phandle_URC(_SIM_COMM_URC_ATTACH);
