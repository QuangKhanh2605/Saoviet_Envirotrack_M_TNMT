...
/*=========== Define =====================*/
#define MAX_LENGTH_SIM_ID   	25
#define MAX_LENGTH_SIM_IMEI   	21 
#define MAX_LENGTH_SIM_CELL   	21 
...
#define SIM_CMD_TIMEOUT1		1000
#define SIM_CMD_TIMEOUT		    10000
#define SIM_CMD_TIMEOUT2 	    5000
#define SIM_TCP_TIMEOUT		    30000
#define SIM_MQTT_TIMEOUT		20000
#define SIM_CGATT_TIMEOUT		2000

#define SIM_CALL_TIMEOUT	    40000
#define SIM_AUDIO_TIMEOUT	    40000
#define SIM_HTTP_TIMEOUT		60000
/* -----------Setting----------*/

#define CID_SERVER              0x32
#define HTTP_HEADER_MAX         3
#define MAX_TIME_RAND_ID        50   //s

/*=========== Struct =====================*/

typedef uint8_t (*CB_AT_Success) (sData *str_Receive, uint16_t pos);
...
typedef struct
{
	char		aSIM_ID[MAX_LENGTH_SIM_ID];  
	uint8_t		RSSI_u8;    
	uint8_t		Ber_u8;
    uint8_t		RSSILevel_u8;  
    char        aIMEI[MAX_LENGTH_SIM_IMEI];
    
    uint16_t    EarFcn_u16;
    uint16_t    EarFcnOffs_u16;
    
    uint16_t    nCellID_u16;
    char        aCELL_ID[MAX_LENGTH_SIM_CELL];
...
{       
    ...
    uint8_t     iGetClock_u8;
    uint32_t    TimeRandID_u32;
}sSimVariable;
...
typedef struct
{
    ...
    void        ( *phandle_Recv ) (uint8_t *pdata, uint16_t length);
...
uint8_t Sim_Common_Get_Sim_ID (sData *uart_string, uint16_t pos);
...
uint8_t Sim_Common_Get_Band (sData *strRecv, uint16_t pos);
...
uint8_t Sim_Common_TCP_Open_2 (sData *strRecv, uint16_t pos);
