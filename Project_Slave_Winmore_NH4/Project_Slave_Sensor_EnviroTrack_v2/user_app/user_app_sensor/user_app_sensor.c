...
static uint8_t fevent_handle_state_sensor(uint8_t event)
{
    ...
    else
    {
        if((sLCD.sScreenNow.Index_u8 == _LCD_SCR_CALIB_NH4) || 
           ...
           (sLCD.sScreenNow.Index_u8 == _LCD_SCR_CALIB_NH4_PLS) || 
           (sLCD.sScreenNow.Index_u8 == _LCD_SCR_CHECK_SETTING && sLCD.sScreenBack.Index_u8 == _LCD_SCR_CALIB_NH4_PLS) ||
...
void Handle_Data_Measure(uint8_t KindRecv)
{
    ...
    switch(KindRecv)
    {
        case _RS485_SS_NH4_OPERA:
            ...
            sCalibPlus.var_x_f = sSensor_NH4.NH4_Value_f;
            sCalibPlus.var_y_f = Cal_CalibPlus(sCalibPlus.var_x_f);
            sSensor_NH4.NH4_Value_f = sCalibPlus.var_y_f;
